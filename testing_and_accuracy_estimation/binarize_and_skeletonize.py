...
def process_prob_raster_to_vector(
    ...
    prune_spurs_len=0.0           # meters; e.g., 1.0 removes ~1 m spurs
):
    """
    Fast vectorizer:
    - optional: treat input as already skeletonized (skip binarize+skelet.)
    - degree-based tracing (no graph build)
    - optional spur pruning by iterative endpoint removal
    - streams to GPKG for speed
    """
    t0 = time.time()
    ...
    vector_output_path = os.path.join(output_folder, f"{name}.shp")  # GPKG by default
    ...
    if input_is_skeleton:
        ...
        skeleton = skeletonize(binary.astype(bool)).astype(np.uint8) if binary.any() else np.zeros_like(binary, dtype=np.uint8)

    # spur pruning (convert meters -> steps)
    if prune_spurs_len and prune_spurs_len > 0:
        # derive pixel size from transform (assumes square-ish pixels)
    ...
    # write skeleton (cheap, useful for QA)
    meta = {
    ...
    # stream write with Fiona (faster than GeoPandas for many small features)
    schema = {
        "geometry": "LineString",
        "properties": {"id": "int"}  # shapefile: include at least one field
    }
    ...
    with fiona.open(
        ...
        for i, geom in enumerate(lines, start=1):
            dst.write({
